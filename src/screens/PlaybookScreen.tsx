...
const playbookTool = new PlaybookTool();

export const PlaybookScreen: React.FC<PlaybookScreenProps> = ({ session, initialAction, onBack, onInputModeChange }) => {
  ...
  const actions = playbookTool.getActions();
  const paramSchema = useMemo(
    () => (selectedAction ? playbookTool.getParamSchema(selectedAction) : []),
    [selectedAction]
  );

  const toolParams = useMemo((): ToolParams => ({
    action: selectedAction,
    ...params,
  }), [selectedAction, params]);

  const command = useMemo(
    () => playbookTool.buildCommand(toolParams),
    [toolParams]
  );

  const errors = useMemo(
    () => playbookTool.validate(toolParams),
    [toolParams]
  );
  ...
  useEffect(() => {
    onInputModeChange?.(phase === 'form' ? 'form' : 'navigate');
  }, [phase, onInputModeChange]);
  ...
  useInput((_input, key) => {
    ...
    if (key.escape) {
      if (phase === 'action') {
        ...
        if (enteredFromSubNav) onBack();
        else setPhase('action');
      } else if (phase === 'preview') {
        setPhase('form');
      } else if (phase === 'execute' && !isRunning) {
        reset();
        setParams({});
        if (enteredFromSubNav) onBack();
        else setPhase('action');
  ...
  if (phase === 'action') {
    return (
      <ToolScreenFrame title="Playbook" subtitle="Select the playbook action" hints={['Enter choose', 'Esc back']}>
        <Box>
          <SelectInput
            items={actions.map((a) => ({ label: a, value: a }))}
            onSelect={(item) => {
              setSelectedAction(item.value);
              setParams({});
              setFocusFieldIndex(0);
              setPhase('form');
            }}
          />
        </Box>
  ...
  if (phase === 'form') {
    return (
      <ToolScreenFrame title="Playbook" subtitle={selectedAction} hints={['Tab next field', 'PgUp/PgDn jump', 'Enter preview', 'Esc back']} status={`field ${Math.min(focusFieldIndex + 1, paramSchema.length)}/${paramSchema.length}: ${currentFieldLabel}`}>
        ...
        {!pickerOpen && errors.length > 0 && (
          <Box marginTop={1} flexDirection="column">
            {errors.map((e) => (
              <Text key={e.field} color="red">
                ✗ {e.message}
              </Text>
            ))}
          </Box>
        ...
        <FieldNavigator
          count={paramSchema.length}
          setFocusIndex={setFocusFieldIndex}
          onEnter={() => setPhase('preview')}
          isActive={phase === 'form' && !pickerOpen}
        />
      </ToolScreenFrame>
    );
  }
  ...
  if (phase === 'preview') {
    return (
      <ToolScreenFrame title="Playbook" subtitle={`${selectedAction} preview`} hints={['Enter run', 'c copy', 'Esc back']}>
        <CommandPreview
          command={command}
          onRun={handleRun}
          onCopy={() => {}}
  ...
  return (
    <ToolScreenFrame title="Playbook" subtitle={`${selectedAction} output`} hints={['↑↓ scroll', 'Space pause', 's save', 'Esc back']}>
      <LiveOutput
        lines={lines}
        isRunning={isRunning}
        exitCode={exitCode}
