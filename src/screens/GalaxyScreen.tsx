...
const galaxyTool = new GalaxyTool();

export const GalaxyScreen: React.FC<GalaxyScreenProps> = ({ session, initialAction, onBack, onInputModeChange }) => {
  ...
  const actions = galaxyTool.getActions();
  const paramSchema = useMemo(() => (selectedAction ? galaxyTool.getParamSchema(selectedAction) : []), [selectedAction]);
  const toolParams = useMemo((): ToolParams => ({ action: selectedAction, ...params }), [selectedAction, params]);
  const command = useMemo(() => galaxyTool.buildCommand(toolParams), [toolParams]);
  const errors = useMemo(() => galaxyTool.validate(toolParams), [toolParams]);
  ...
  if (phase === 'action') return (<ToolScreenFrame title="Galaxy" subtitle="Select the galaxy action" hints={['Enter choose', 'Esc back']}><Box marginTop={1}><SelectInput items={actions.map(a => ({ label: a, value: a }))} onSelect={item => { setSelectedAction(item.value); setParams({}); setFocusFieldIndex(0); setPhase('form'); }} /></Box></ToolScreenFrame>);

  if (phase === 'form') return (
    <ToolScreenFrame title="Galaxy" subtitle={selectedAction} hints={['Tab next field', 'PgUp/PgDn jump', 'Enter preview', 'Esc back']} status={`field ${Math.min(focusFieldIndex + 1, paramSchema.length)}/${paramSchema.length}: ${currentFieldLabel}`}>
  ...
  if (phase === 'preview') return (<ToolScreenFrame title="Galaxy" subtitle={`${selectedAction} preview`} hints={['Enter run', 'c copy', 'Esc back']}><CommandPreview command={command} onRun={handleRun} onCopy={() => {}} onBack={() => setPhase('form')} isActive={phase === 'preview'} /></ToolScreenFrame>);

  return (<ToolScreenFrame title="Galaxy" subtitle={`${selectedAction} output`} hints={['↑↓ scroll', 'Space pause', 's save', 'Esc back']}><LiveOutput lines={lines} isRunning={isRunning} exitCode={exitCode} durationMs={durationMs} onSave={() => {}} isActive={phase === 'execute'} /></ToolScreenFrame>);
};
