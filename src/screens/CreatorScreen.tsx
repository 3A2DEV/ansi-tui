...
interface CreatorScreenProps { session: Session | null; initialAction?: string; onBack: () => void; onInputModeChange?: (mode: InputMode) => void; }
type Phase = 'action' | 'form' | 'preview' | 'execute';

const creatorTool = new CreatorTool();

export const CreatorScreen: React.FC<CreatorScreenProps> = ({ session, initialAction, onBack, onInputModeChange }) => {
  ...
  const actions = creatorTool.getActions();
  const paramSchema = useMemo(() => selectedAction ? creatorTool.getParamSchema(selectedAction) : [], [selectedAction]);
  const toolParams = useMemo((): ToolParams => ({ action: selectedAction, ...params }), [selectedAction, params]);
  const command = useMemo(() => creatorTool.buildCommand(toolParams), [toolParams]);
  const errors = useMemo(() => creatorTool.validate(toolParams), [toolParams]);
  ...
  if (phase === 'action') return (<ToolScreenFrame title="Creator" subtitle="Select the creator action" hints={['Enter choose', 'Esc back']}><Box marginTop={1}><SelectInput items={actions.map(a => ({ label: a, value: a }))} onSelect={item => { setSelectedAction(item.value); setParams({}); setFocusFieldIndex(0); setPhase('form'); }} /></Box></ToolScreenFrame>);

  if (phase === 'form') return (
    <ToolScreenFrame title="Creator" subtitle={selectedAction} hints={['Tab next field', 'PgUp/PgDn jump', 'Enter preview', 'Esc back']} status={`field ${Math.min(focusFieldIndex + 1, paramSchema.length)}/${paramSchema.length}: ${currentFieldLabel}`}>
  ...
  if (phase === 'preview') return (<ToolScreenFrame title="Creator" subtitle={`${selectedAction} preview`} hints={['Enter run', 'c copy', 'Esc back']}><CommandPreview command={command} onRun={handleRun} onCopy={() => {}} onBack={() => setPhase('form')} isActive={phase === 'preview'} /></ToolScreenFrame>);

  return (<ToolScreenFrame title="Creator" subtitle={`${selectedAction} output`} hints={['↑↓ scroll', 'Space pause', 's save', 'Esc back']}><LiveOutput lines={lines} isRunning={isRunning} exitCode={exitCode} durationMs={durationMs} onSave={() => {}} isActive={phase === 'execute'} /></ToolScreenFrame>);
};
