...
interface InventoryScreenProps { session: Session | null; initialAction?: string; onBack: () => void; onInputModeChange?: (mode: InputMode) => void; }
type Phase = 'action' | 'form' | 'preview' | 'execute';

const inventoryTool = new InventoryTool();

export const InventoryScreen: React.FC<InventoryScreenProps> = ({ session, initialAction, onBack, onInputModeChange }) => {
  ...
  const actions = inventoryTool.getActions();
  const paramSchema = useMemo(() => selectedAction ? inventoryTool.getParamSchema(selectedAction) : [], [selectedAction]);
  const toolParams = useMemo((): ToolParams => ({ action: selectedAction, ...params }), [selectedAction, params]);
  const command = useMemo(() => inventoryTool.buildCommand(toolParams), [toolParams]);
  const errors = useMemo(() => inventoryTool.validate(toolParams), [toolParams]);
  ...
  if (phase === 'action') return (<ToolScreenFrame title="Inventory" subtitle="Select the inventory action" hints={['Enter choose', 'Esc back']}><Box marginTop={1}><SelectInput items={actions.map(a => ({ label: a, value: a }))} onSelect={item => { setSelectedAction(item.value); setParams({}); setFocusFieldIndex(0); setPhase('form'); }} /></Box></ToolScreenFrame>);

  if (phase === 'form') return (
    <ToolScreenFrame title="Inventory" subtitle={selectedAction} hints={['Tab next field', 'PgUp/PgDn jump', 'Enter preview', 'Esc back']} status={`field ${Math.min(focusFieldIndex + 1, paramSchema.length)}/${paramSchema.length}: ${currentFieldLabel}`}>
  ...
  if (phase === 'preview') return (<ToolScreenFrame title="Inventory" subtitle={`${selectedAction} preview`} hints={['Enter run', 'c copy', 'Esc back']}><CommandPreview command={command} onRun={handleRun} onCopy={() => {}} onBack={() => setPhase('form')} isActive={phase === 'preview'} /></ToolScreenFrame>);

  return (<ToolScreenFrame title="Inventory" subtitle={`${selectedAction} output`} hints={['↑↓ scroll', 'Space pause', 's save', 'Esc back']}><LiveOutput lines={lines} isRunning={isRunning} exitCode={exitCode} durationMs={durationMs} onSave={() => {}} isActive={phase === 'execute'} /></ToolScreenFrame>);
};
